...
class arXmlExportSingleFileJob extends arBaseJob
{
  ...
  public function runJob($parameters)
  {
    ...
    $this->checkDestinationDirExistsOrCreateIt();
    $this->exportResource();
  ...
  private function checkDestinationDirExistsOrCreateIt()
  {
  ...
  protected function exportResource()
  {
    ...
    // Copy temp file to destination location and remote temp file
    copy($tempFilePath, self::getPath($this->params['objectId'], $this->params['format']));
    $skipLines = ($this->params['format'] == 'ead') ? 2 : 1;
    $this->writeXmlFileSkippingLines($tempFilePath, self::getPath($this->params['objectId'], $this->params['format'], true), $skipLines);
  ...
  public function writeXmlFileSkippingLines($source, $destination, $skipLines = 0)
  {
    $sfp = fopen($source, 'r');
